...
<div id="job-executions">
	...
	<#assign executions_url><@spring.url relativeUrl="/batch/jobs/executions"/></#assign>
	...
	<#if jobExecutions?? && jobExecutions?size!=0>
		...
		<table title="Recent and Current Job Executions"
			...
			<#list jobExecutions as execution>
				...
				<tr class="${rowClass}">
					<#assign execution_url><@spring.url relativeUrl="/batch/jobs/executions/${execution.id?c}"/></#assign>
					...
					<#assign step_url><@spring.url relativeUrl="/batch/jobs/executions/${execution.id?c}/steps/${stepExecution.id?c}/progress"/></#assign>
