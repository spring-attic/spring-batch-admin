...
public class MessagesHolderHandlerTests {

	private MessagesHolderHandler handler = new MessagesHolderHandler();
	...
	public void testHandleMessageTimestamp() throws Exception {
		...
		Collection<Message<?>> messages = handler.getMessages();
		assertEquals(1, messages.size());
		Message<?> message = messages.iterator().next();
		assertEquals(message.getHeaders().getTimestamp(), message.getHeaders().get("$timestamp"));
	}
