...
public class BatchMBeanExporterTests {
	...
	public void testJobObjectNameStaticProperties() throws Exception {
		...
		assertEquals("org.springframework.batch:type=JobExecution,name=job,foo=bar", exporter.getBeanKeyForJobExecution("job"));
	}

	@Test
	public void testStepObjectNameStaticProperties() throws Exception {
		exporter.setObjectNameStaticProperties(Collections.singletonMap("foo", "bar"));
		assertEquals("org.springframework.batch:type=JobExecution,name=job,step=step,foo=bar", exporter.getBeanKeyForStepExecution("job", "step"));
	}
