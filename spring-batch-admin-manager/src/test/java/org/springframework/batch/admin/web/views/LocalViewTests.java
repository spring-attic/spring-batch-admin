...
public class LocalViewTests extends AbstractManagerViewTests {
	...
	public void testLocalViewWithStatus() throws Exception {
		model.put("serviceStatus", new StepServiceStatus(false,0,false));
		model.put(BindingResult.MODEL_KEY_PREFIX + "serviceStatus", new MapBindingResult(model, "serviceStatus"));
		local.render(model, request, response);
		String content = response.getContentAsString();
		assertTrue(content.contains("Status: available=false"));
	}
